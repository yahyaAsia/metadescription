...
def fetch_page_content(url, max_retries=2):
    """Fetch content using sumy's HtmlParser, fall back to requests if needed."""
    # Try HtmlParser first
    try:
        parser = HtmlParser.from_url(url, Tokenizer("english"))
        logger.debug(f"Successfully fetched {url} with HtmlParser")
        return parser.document, None
    except Exception as e:
        logger.warning(f"HtmlParser failed for {url}: {e}. Falling back to requests.")
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            logger.debug(f"Successfully fetched {url} with requests: {len(page_text)} characters")
            return PlaintextParser.from_string(page_text, Tokenizer("english")).document, None
...
def generate_meta_descriptions(urls):
    ...
    for i, url in enumerate(urls):
        try:
            ...
            # Fetch page content
            document, fetch_error = fetch_page_content(url)
            if not document:
                raise ValueError(f"Failed to fetch page content: {fetch_error}")

            # Extract text for keyword analysis
            page_text = ' '.join([sentence._text for sentence in document.sentences])
            logger.debug(f"Extracted {len(page_text)} characters for {url}")
            ...
            summary = summarizer(document, 3)  # 3 sentences for richer summary
...
if uploaded_file is not None:
    try:
        ...
        # Check for requirements.txt-like content
...
- Ensure the uploaded `urls.txt` file contains valid URLs.
- If errors persist, test locally and provide the CSV output and logs to support.
""")
